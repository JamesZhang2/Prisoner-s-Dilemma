...
'Modified by James Ruogu Zhang on July 20, 2019
...
Public Class Form1
    ...
    Private Sub ButtonNext1_Click(sender As Object, e As EventArgs) Handles ButtonNext1.Click
        ShowDescription2()
        round = 2
        HadBetrayed = False  'Reset HadBetrayed for Detective
    End Sub
    ...
    Sub Grudger()
        If turn(1) = 1 Then
            OppChoice(1, turn(1)) = 1                     'Cooperates in the first turn
        Else
            If YourChoice(1, turn(1) - 1) = 0 Then        'You betrayed last turn
                HadBetrayed = True
            End If
            If HadBetrayed Then
                OppChoice(1, turn(1)) = 0                 'Betrays if you had betrayed before
            Else
                OppChoice(1, turn(1)) = 1                 'Cooperates if you hadn't betrayed before
            End If
        End If
    End Sub

    Sub Cheater()
        OppChoice(2, turn(2)) = 0                         'Always betrays
    End Sub

    Sub Detective()                                       'Plays CBCC first, and then copycat if you had betrayed, cheater if you hadn't
        If turn(3) > 1 And YourChoice(3, turn(3) - 1) = 0 Then
            HadBetrayed = True
        End If
        If turn(3) = 1 Then
            OppChoice(3, turn(3)) = 1
        ElseIf turn(3) = 2 Then
            OppChoice(3, turn(3)) = 0
        ElseIf turn(3) = 3 Then
            OppChoice(3, turn(3)) = 1
        ElseIf turn(3) = 4 Then
            OppChoice(3, turn(3)) = 1
        Else
            If HadBetrayed = False Then
                OppChoice(3, turn(3)) = 0
            Else
                OppChoice(3, turn(3)) = YourChoice(3, turn(3) - 1)
            End If
        End If
    End Sub
    ...
    Sub Copycat()
        If turn(4) = 1 Then
            OppChoice(4, turn(4)) = 1                     'Cooperates in the first turn
        Else                                              'Copies your previous choice in later turns
            OppChoice(4, turn(4)) = YourChoice(4, turn(4) - 1)
        End If
    End Sub

    Sub Cooperator()
        OppChoice(5, turn(5)) = 1                         'Always cooperates
    End Sub
